...
| 私聊建群会话选择 | 建群时可选“新建会话/绑定已有会话”；可输入初始需求，建群后自动发给 AI 并根据内容命名群组和会话 | `/create_chat`、`/建群` |
...
| `/create_chat` / `/建群` | 私聊中调出建群卡片；可填写初始需求，建群后自动发给 AI 并根据内容命名群组和会话 |
...
- 初始需求输入框（可选，最多 2000 字）：有内容时建群完成后自动发给 OpenCode，AI 处理完毕后通过 `session.idle` 事件将生成的会话名称同步到飞书群名和本地缓存。
- 自动命名仅在新建会话且未自定义群名时触发；绑定已有会话或用户已指定群名时跳过命名同步。
